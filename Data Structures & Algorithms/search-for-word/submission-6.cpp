...
public:
    ...
    void dfs(int i, int j, vector<vector<char>>& board, string word, string temp,vector<vector<int>>& visited){
        if( (i>=0 && i < board.size()) && (j >=0 && j < board[0].size()) && temp.length() > word.length()){
            return;
        }
        ...
        //up
        if( (i-1>=0 && i-1 < board.size()) && (j >=0 && j < board[0].size())&& visited[i-1][j] == -1){
        ...
        //left
        if( (i>=0 && i < board.size()) && (j-1 >=0 && j-1 < board[0].size())&& visited[i][j-1] == -1){
